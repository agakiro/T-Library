...
import presentation.viewmodel.ItemInformationFragment
import presentation.viewmodel.ItemListFragment
import presentation.viewmodel.ItemViewModel
import presentation.viewmodel.ItemViewModelFactory
...
import data.local.LibraryDatabase
...
class MainActivity : AppCompatActivity() {
    ...
    private val viewModel: ItemViewModel by viewModels {
        ItemViewModelFactory(
            ItemRepositoryImpl(
                LibraryDatabase.getDb(this).getDao(),
            SortSettings(this)
        )
        )
    }
        ...
        if (isPortrait) {
            supportFragmentManager
                ...
                .replace(R.id.fragment_container,
                    ItemInformationFragment.newInstance(null, true, viewModel.addingItemType.value)
                )
                .addToBackStack(null)
                .commit()
        } else {
            supportFragmentManager
                .beginTransaction()
                .replace(R.id.information_fragment_container,
                    ItemInformationFragment.newInstance(null, true, viewModel.addingItemType.value)
                )
                .commit()
        }
